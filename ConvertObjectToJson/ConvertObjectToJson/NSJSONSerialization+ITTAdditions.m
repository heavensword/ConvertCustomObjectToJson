//
//  NSJSONSerialization+ITTAdditions.m
//  ConvertObjectToJson
//
 ...
 *   etc. The super calss of custom object must be ITTBaseModelObject
 ...
 *  The super class of custom object must be ITTBaseModelObject
 *	\returns A string of json format
 */
	...
	for (id object in array) {
		if (object) {
			if ([object isKindOfClass:[ITTBaseModelObject class]]) {
				NSDictionary *propertiesValuesDic = [object propertiesAndValuesDic];
	...
	for (NSString *key in allKeys) {
		...
		if (value) {
			if ([value isKindOfClass:[ITTBaseModelObject class]]) {
				...
				[self serializeDictionary:[value propertiesAndValuesDic] jsonString:string];
			}
